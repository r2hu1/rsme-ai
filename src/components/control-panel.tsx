...
export function ControlPanel({
	...
	const handleItemRemove = (
		...
		const updatedArray = (currentValues[fieldName] as any[]).filter(
			(_, i) => i !== index,
		);
			...
			<Accordion type="multiple" defaultValue={["import"]} className="w-full">
				<AccordionItem value="import">
					...
					<AccordionContent className="pt-2">
						<div className="space-y-8">
							<Card>
								<CardContent className="p-4 space-y-6 -mt-6">
									<input
										type="file"
										ref={fileInputRef}
									...
									/>
									<Button
										onClick={() => onParse(importText)}
										disabled={loading === "parse" || !importText}
										className="w-full"
									>
										{loading === "parse" ? (
											<Loader2 className="mr-2 h-4 w-4 animate-spin" />
							...
							<Card>
								<CardContent className="p-4 space-y-3">
									...
									<Button
										...
										disabled={loading === "generate" || !generatePrompt}
										className="w-full"
									>
										{loading === "generate" ? (
											<Loader2 className="mr-2 h-4 w-4 animate-spin" />
												...
												<div className="col-span-2 flex items-center gap-2">
													<Slider
														...
														onValueChange={(values) =>
															handleThemeChange("borderWidth", values[0])
														}
														min={0}
														max={5}
														step={0.5}
									...
									<FormField
										...
										render={({ field }) => (
											<FormItem>
												...
												<FormControl>
													<Input {...field} onBlur={() => handleBlur("name")} />
												</FormControl>{" "}
											</FormItem>
										)}
									/>
									...
									{expFields.map((field, index) => (
										<Card key={field.id} className="p-4 relative">
											<Button
												...
												onClick={() =>
													handleItemRemove(removeExp, index, "experience")
												}
											>
									...
									{eduFields.map((field, index) => (
										<Card key={field.id} className="p-4 relative">
											<Button
												...
												className="absolute top-2 right-2 h-6 w-6"
												onClick={() =>
													handleItemRemove(removeEdu, index, "education")
												}
											>
												<Trash2 className="h-4 w-4 text-destructive" />
											</Button>
									...
									{projectFields.map((field, index) => (
										<Card key={field.id} className="p-4 relative">
											<Button
												...
												className="absolute top-2 right-2 h-6 w-6"
												onClick={() =>
													handleItemRemove(removeProject, index, "projects")
												}
											>
												<Trash2 className="h-4 w-4 text-destructive" />
											</Button>
								...
								<CardContent>
									<div className="flex flex-wrap gap-2">
										{skillFields.map((field, index) => (
											<div
												key={field.id}
												className="flex items-center gap-1 bg-secondary rounded-full"
											>
												<Controller
													name={`skills.${index}`}
													control={form.control}
													render={({ field }) => (
														<Input
															...
															className="h-8 bg-transparent border-none focus-visible:ring-0 w-32"
														/>
													)}
												/>
												<Button
													...
													onClick={() =>
														handleItemRemove(removeSkill, index, "skills")
													}
												>
				...
				<AccordionItem value="score">
					...
					<AccordionContent className="pt-2">
						<div className="space-y-4">
							<p className="text-sm text-muted-foreground">
								Paste a job description to score your skills based on market
								relevance.
							</p>
							<Textarea
							...
							/>
							<Button
								onClick={() => onScoreSkills(jobDescription)}
								disabled={loading === "score" || !jobDescription}
								className="w-full"
							>
								{loading === "score" ? (
									<Loader2 className="mr-2 h-4 w-4 animate-spin" />
							...
							{skillScores && (
								<Card>
									<CardHeader>
										<CardTitle>Skill Relevance</CardTitle>
									</CardHeader>
