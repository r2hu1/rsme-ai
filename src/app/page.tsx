...
import { generateResumeFromInput } from "@/ai/flows/generate-resume-from-input";
...
import type { ResumeData, SkillScore, ContentEvaluation } from "@/lib/types";
...
const initialResume: ResumeData = {
	...
	experience: [
		...
		},
		{
			...
			dates: "2018 - 2020",
			description:
				"Developed and maintained front-end features for a large-scale e-commerce platform using React and Redux.",
		},
	],
	education: [
		{
	...
	projects: [
		{
			...
			description:
				"Built a high-performance search engine for an e-commerce site using Elasticsearch, resulting in a 30% increase in conversion rates.",
			dates: "2022",
		...
		},
		{
			...
			description:
				"Designed and developed a real-time data visualization dashboard with D3.js and React.",
			dates: "2021",
...
export default function Home() {
	...
	const [analyzedResumeContent, setAnalyzedResumeContent] = useState<
		string | null
	>(null);
	...
	const handleGenerateResume = async (prompt: string) => {
		setLoading("generate");
		try {
			const generatedData = await generateResumeFromInput({ prompt });
			...
			toast({
				...
				description: "Your new resume has been generated by AI.",
			});
		} catch (error) {
			console.error(error);
			toast({
	...
	const handleScoreSkills = async (jobDescription: string) => {
		...
		try {
			...
			toast({
				title: "Skills Scored",
				description:
					"Your skills have been scored against the job description.",
			});
		} catch (error) {
			console.error(error);
			toast({
